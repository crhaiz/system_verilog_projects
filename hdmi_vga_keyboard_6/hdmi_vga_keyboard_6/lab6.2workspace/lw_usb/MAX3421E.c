...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int n  = XSpi_Transfer(&SpiInstance, datain, NULL, ByteCount);
	if (n != 0){
		xil_printf("error\n");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
	XSpi_SetSlaveSelect(&SpiInstance, 1);
		BYTE datain[1+nbytes];
		datain[0] = reg+2;
		unsigned int ByteCount = nbytes + 1;
...
BYTE MAXreg_rd(BYTE reg) {
	...
	datain[0] = reg;
	datain[1] = 0;
	int n = XSpi_Transfer(&SpiInstance, datain, dataout, ByteCount);
	if (n != 0){
		xil_printf("error\n");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	unsigned int ByteCount = 1 + nbytes;
		BYTE datain[1 + nbytes];
		BYTE dataout[1 + nbytes];
		datain[0] = reg;
		int n = XSpi_Transfer(&SpiInstance, datain, dataout, ByteCount);
		if (n != 0){
			xil_printf("error\n");
