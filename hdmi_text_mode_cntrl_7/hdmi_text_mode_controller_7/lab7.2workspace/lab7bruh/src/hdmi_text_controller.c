...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
	...
	if (color % 2) {
		hdmi_ctrl->palette[color/2] = hdmi_ctrl->palette[color/2] & 0b00000000000000000001111111111110; // 1FFE
		hdmi_ctrl->palette[color/2] |= (red<<8 | green<<4|blue<<0)<<13;


	}
	else {
		hdmi_ctrl->palette[color/2] = hdmi_ctrl->palette[color/2] & 0b00000001111111111110000000000000; // FFE1
		hdmi_ctrl->palette[color/2] |= (red<<8 | green<<4|blue<<0)<<1;
	}
}
